  ...
  >
    <div class="confirm_container">
      ...
      <div class="confirm_container_cta">
        ...
        <gf-accept-button
          :label="'Yes, delete'"
          @click.prevent="deleteDigestGroup()"
        ></gf-accept-button>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    closeTab(deleted) {
      console.log("Cerrando tab:", deleted);
      if (deleted === true) {
        this.$emit("closePopupDeleted", true);
      } else {
        this.$emit("closePopup", false);
      }
    },
    async deleteDigestGroup() {
      let url = this.$store.state.gf_contracts.backend;
      await axios
        .delete(url + "/digest_groups/" + this.post_to_delete.id + "/delete")
        .then(() => {
          const notification = {
            type: "success",
            message: "Your digest group was successfully deleted!",
          };
