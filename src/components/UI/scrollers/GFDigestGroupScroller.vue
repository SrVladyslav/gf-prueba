<template>
  <div class="sg_container" v-if="posts.length > 0">
    <div
      class="sg_scroller_container"
      ref="sg_scroller_container"
      @wheel.prevent="scrollHorizontal($event, 'sg_scroller_container')"
    >
      <div class="sg_scroller_slider">
        <div
          class="sg_digest_post_info"
          v-for="(post, key) in posts"
          :key="key"
        >
          <div class="sg_digest_info_container">
            <div class="sg_digest_post_img">
              <img
                :src="this.$store.state.gf_contracts.backend + post.banner"
                alt=""
              />
            </div>
            <div class="sg_digest_text">
  ...
  <div class="sg_container" v-else>
    <div class="sg_digest_text_f">
      This digest does not contain any groups yet.
    </div>
  </div>
</template>
...
.sg_scroller_container {
  position: relative;
  width: 100%;
  height: fit-content;
  overflow-x: scroll;
}
.sg_scroller_container::-webkit-scrollbar {
  width: 10px;
  height: 10px;
  background: white;
  cursor: pointer;
...
.sg_scroller_container::-webkit-scrollbar-thumb {
  background: #6e6e75;
...
@media (hover: hover) {
  .sg_digest_post_info:hover {
    padding: 0px 0px 3px 0px;
  }
  .sg_digest_info_container:hover {
